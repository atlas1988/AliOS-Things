...
static void sns_cam_refresh_task(void *arg);
static int get_cam_data(float *x, float *y, float *z);
...
static void cam_delayed_action(void *arg)
{
	...
	aos_post_delayed_action(500, cam_delayed_action, NULL);
}

char cam_test(void *item)
{
	char ret = 0, i = 0;
	...
	for(i = 0;test_item[i].id != NULL ;i++){
		if(strcmp(test_item[i].name,"cam") == 0){
			item_select = i;
			break;
		}
	}

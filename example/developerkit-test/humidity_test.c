...
static void sns_humi_refresh_task(void *arg);
 static int get_humi_data(float *x, float *y, float *z);

 char humi_test(void *item)
{
	char ret = 0, i = 0;
	aos_msleep(100);
	test_item =(test_items_t *) item;
	for(i = 0;test_item[i].id != NULL ;i++){
		if(strcmp(test_item[i].name,"humi") == 0){
			item_select = i;
			break;
		}
	}
    /* open humi sensor */
    fd_humi = aos_open(test_item[item_select].device_path, O_RDWR);
    ...
    aos_timer_new(&refresh_timer, sns_humi_refresh_task, NULL, 350, 1);
	return ret;
}
...
static int get_humi_data(float *x, float *y, float *z)
{
	...
	printf("get_humi_data proxmity = %d \n" ,data.h);
    *x = (float)data.h / 100.0f  ;
    *y = 0  ;
    *z = 0  ;
...
static void sns_humi_refresh_task(void *arg){
	...
	if(task_count == 1){
		...
		get_humi_data(&x, &y, &z);
		printf("humi sensor data x = %.4f, y = %.4f,z = %.4f \n",x,y,z);

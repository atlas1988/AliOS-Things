...
static void sns_pls_refresh_task(void *arg);
 static int get_pls_data(float *x, float *y, float *z);

 char pls_test(void *item)
{
	char ret = 0, i = 0;
	aos_msleep(100);
	test_item =(test_items_t *) item;
	for(i = 0;test_item[i].id != NULL ;i++){
		if(strcmp(test_item[i].name,"pls") == 0){
			item_select = i;
			break;
		}
	}
    /* open pls sensor */
    fd_pls = aos_open(test_item[item_select].device_path, O_RDWR);
	printf("device path %s \n",test_item[item_select].device_path);
    if (fd_pls < 0) {
        printf("pls sensor open failed !\n");
		return -1;
    }
    ...
    aos_timer_new(&refresh_timer, sns_pls_refresh_task, NULL, 350, 1);
	return ret;
}
...
static int get_pls_data(float *x, float *y, float *z)
{
    ...
    size = aos_read(fd_pls, &data, sizeof(data));
    if (size != sizeof(data)) {
        return -1;
    }
	printf("get_pls_data proxmity = %d \n" ,data.present);
    *x = (float)data.present   ;
    *y = 0  ;
    *z = 0  ;
...
static void sns_pls_refresh_task(void *arg){
	...
	if(task_count == 1){
		clean_labels();
		chart_label_init_one(10);
	}else if(task_count >= 2){
		get_pls_data(&x, &y, &z);
		printf("pls sensor data x = %.4f, y = %.4f,z = %.4f \n",x,y,z);

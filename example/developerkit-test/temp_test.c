...
static void sns_temp_refresh_task(void *arg);
 static int get_temp_data(float *x, float *y, float *z);

 char temp_test(void *item)
{
	char ret = 0, i = 0;
	aos_msleep(100);
	test_item =(test_items_t *) item;
	for(i = 0;test_item[i].id != NULL ;i++){
		if(strcmp(test_item[i].name,"temp") == 0){
			item_select = i;
			break;
		}
	}
    /* open temp sensor */
    fd_temp = aos_open(test_item[item_select].device_path, O_RDWR);
    ...
    aos_timer_new(&refresh_timer, sns_temp_refresh_task, NULL, 350, 1);
	return ret;
}
...
static int get_temp_data(float *x, float *y, float *z)
{
	...
	printf("get_temp_data proxmity = %d \n" ,data.t);
    *x = (float)data.t /100.0f ;
    *y = 0  ;
    *z = 0  ;
...
static void sns_temp_refresh_task(void *arg){
	...
	if(task_count == 1){
		clean_labels();
		chart_label_init_one(100);
	}else if(task_count >= 2){
		get_temp_data(&x, &y, &z);
		printf("temp sensor data x = %.4f, y = %.4f,z = %.4f \n",x,y,z);
